...
from core.data_fetcher import fetch_cumulative_blame_data, fetch_overview_data, fetch_tags_data
...
class TestDataFetcher(unittest.TestCase):
    ...
    def test_active_branches_handles_problematic_branch_data(self, mock_logger):
        ...
        mock_base_commits = pd.DataFrame(
            {"committer": ["Test User"], "author": ["Test User"], "message": ["Test commit"]}, index=[future_time]
        ...
        mock_branch_commits_empty = pd.DataFrame({"author": []}, index=pd.DatetimeIndex([]))

        # 2. Normal branch commits that will work
        mock_branch_commits_normal = pd.DataFrame({"author": ["Test User"]}, index=[future_time])
        ...
        self.assertTrue(
            any("git_error_branch" in str(args[0]) for args in mock_logger.warning.call_args_list),
            "No warning log for git_error_branch found",
        )
        self.assertTrue(
            any("exception_branch" in str(args[0]) for args in mock_logger.warning.call_args_list),
    ...
    def test_active_branches_empty_list_handling(self, mock_logger):
        ...
        mock_base_commits = pd.DataFrame(
            {"committer": ["Test User"], "author": ["Test User"], "message": ["Test commit"]},
    ...
    def test_cumulative_blame_direct_keyerror(self, mock_logger):
        ...
        self.assertTrue(
            any("'rev'" in str(args[0]) for args in mock_logger.exception.call_args_list),
    ...
    def test_cumulative_blame_cache_keyerror(self, mock_logger):
        ...
        mock_df = pd.DataFrame(
            {"date": pd.date_range(start="2023-01-01", periods=3), "author1": [10, 20, 30], "author2": [5, 10, 15]}
        )

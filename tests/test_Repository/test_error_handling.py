...
class TestRepositoryErrorHandling:
    ...
    def test_file_change_history_memory_error_simulation(self, temp_repo):
        ...
        with patch.object(repo, 'file_change_history') as mock_fch:
            ...
            expected_columns = [
                "file", "unique_committers", "abs_rate_of_change", "net_rate_of_change",
                "net_change", "abs_change", "edit_rate", "lines", "repository"
            ]
    ...
    def test_punchcard_missing_columns(self, temp_repo):
        ...
        with patch.object(repo, 'punchcard') as mock_punchcard:
            mock_punchcard.return_value = pd.DataFrame(columns=[
                "hour_of_day", "day_of_week", "lines", "insertions", "deletions", "net"
            ])
    ...
    def test_file_change_rates_empty_repository(self, temp_repo):
        ...
        with patch.object(repo, 'file_change_history') as mock_fch:
            mock_fch.return_value = pd.DataFrame(columns=[
                "filename", "insertions", "deletions", "lines", "message", "committer", "author"
            ])

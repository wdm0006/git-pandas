...
from gitpandas.cache import CacheMissError, EphemeralCache, multicache, RedisDFCache
...
class TestRedisDFCache:
    ...
    def test_set_get(self, mock_redis):
        ...
        with patch('pickle.dumps', return_value=mock_pickle_data) as mock_dumps:
            with patch('pickle.loads', return_value=df) as mock_loads:
                cache = RedisDFCache()
                ...
                mock_redis_instance.set.assert_called_once_with(
                    'gitpandas_key1', mock_pickle_data, ex=None
                )

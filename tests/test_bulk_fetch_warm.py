...
class TestBulkFetchAndWarm(unittest.TestCase):
    ...
    def test_bulk_fetch_and_warm_custom_parameters(self):
        ...
        self.mock_repo1.safe_fetch_remote.return_value = {'success': True, 'remote_exists': True, 'changes_available': False}
        self.mock_repo1.warm_cache.return_value = {'success': True, 'cache_entries_created': 1}
        self.mock_repo2.safe_fetch_remote.return_value = {'success': True, 'remote_exists': False, 'changes_available': False}
        self.mock_repo2.warm_cache.return_value = {'success': True, 'cache_entries_created': 0}
        self.mock_repo3.safe_fetch_remote.return_value = {'success': True, 'remote_exists': True, 'changes_available': True}
        ...
        self.mock_repo1.safe_fetch_remote.assert_called_once_with(
            remote_name='upstream',
            prune=True,
            dry_run=True
        )
        self.mock_repo1.warm_cache.assert_called_once_with(
            methods=['branches', 'tags'],
            limit=50
        )
        
        # Also verify repo2 and repo3 got the same parameters
        self.mock_repo2.safe_fetch_remote.assert_called_once_with(
            remote_name='upstream',
            prune=True,
            dry_run=True
        )
        self.mock_repo3.safe_fetch_remote.assert_called_once_with(
            remote_name='upstream',
            prune=True,
            dry_run=True
        )
    ...
    def test_bulk_fetch_and_warm_parallel_execution(self):
        ...
        with patch('gitpandas.project._has_joblib', True), \
             patch('joblib.Parallel', mock_parallel_class) as mock_parallel, \
             patch('joblib.delayed', Mock()) as mock_delayed:

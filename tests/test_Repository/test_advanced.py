...
class TestRepositoryAdvanced:
    ...
    def test_parallel_cumulative_blame(self, local_repo):
        ...
        with patch('gitpandas.repository._has_joblib', True):
            with patch('gitpandas.repository.Parallel') as mock_parallel:
                # Setup mock return values
                mock_blame1 = pd.DataFrame({"loc": {"Test User": 3}})
                mock_blame2 = pd.DataFrame({"loc": {"Test User": 5}})
                ...
                # Call the function
                result = repo.parallel_cumulative_blame(
                    branch="master", 
                    workers=2
                )
